...
import {
  ...
  COLLEGE_COLUMNS,
  COLLEGE_DATA,
  COLLEGE_EXAMPLE_COLUMNS,
  COLLEGE_EXAMPLE_DATA,
  OTHER_CALCULATORS
} from '../../constants';
...
class College extends Component {
  ...
  calculateGPA = (credits, letterGrades) => {
    ...
    console.log('GPA: ', gpa);
    return gpa.toFixed(DECIMAL_LENGTH);
  };
  ...
  showGPAModal = gpa => {
    ...
    if (gpa < 2) {
      ...
      this.setState({ modalClass: NICE, modalTitle: 'Nice!' });
    } else if (gpa >= 3 && gpa < 3.5) {
      this.setState({ modalClass: GOOD, modalTitle: 'Keep doing this!' });
    } else {
      this.setState({ modalClass: SUCCESS, modalTitle: 'Amazing!' });
    }
  };
            ...
            <Form.Item required={true} key={k}>
              {getFieldDecorator(`letterGrades[${k}]`, {
                ...
                <Select placeholder="Letter Grade">
                  {LETTER_GRADES.map((letterGrade, index) => (
        ...
        <div className="college-wrapper">
          <Row gutter={32}>
            <Col span={15}>
              ...
              <Table
                columns={COLLEGE_EXAMPLE_COLUMNS}
                dataSource={COLLEGE_EXAMPLE_DATA}
                pagination={false}
              />
              <p>
